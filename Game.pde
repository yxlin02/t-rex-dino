...
// Visual elements
color skyColor = color(135, 206, 235); // Sky blue
color groundColor = color(139, 69, 19); // Brown ground
color dinoColor = color(0, 100, 0); // Dark green
...
void drawMenu() {
  ...
  textSize(14);
  text("High Score: " + int(highScore), width/2, height/2 + 80);
}
...
void playGame() {
  ...
  totalScore += 0.1; // Cumulative score across levels
  ...
  if (obstacleTimer >= obstacleInterval) {
    ...
    if (dinoDistance > 120) { // Only spawn if dinosaur has enough space
      ...
      int chunkSize = int(random(1, 4)); // 1, 2, or 3 cacti in chunk
      ...
      if (chunkSize == 1) {
        obstacleInterval = int(random(80, 120)) - (currentLevel - 1) * 10;
      } else if (chunkSize == 2) {
        obstacleInterval = int(random(100, 140)) - (currentLevel - 1) * 10;
      } else { // chunkSize == 3
        obstacleInterval = int(random(120, 160)) - (currentLevel - 1) * 10;
      }
...
void drawGameOver() {
  ...
  textSize(18);
  text("Level Score: " + int(score), width/2, height/2);
  text("Total Score: " + int(totalScore), width/2, height/2 + 25);
...
void drawUI() {
  ...
  textAlign(LEFT);
  textSize(16);
  text("Level Score: " + int(score), 20, 30);
  text("Total Score: " + int(totalScore), 20, 50);
  ...
  if (debugMode) {
    ...
    text("Dino Size: " + int(dino.size), 20, 160);
...
void restartGame() {
  ...
  score = 0;
  levelTime = 0; // Reset level time
  gameSpeed = 5 + (currentLevel - 1) * 1.5; // Increase speed for higher levels
...
void nextLevel() {
  ...
  if (currentLevel <= maxLevels) {
    ...
    gameSpeed = 5 + (currentLevel - 1) * 1.5;
    obstacleInterval = max(60, 120 - (currentLevel - 1) * 20);
    obstacles.clear();
    dino.reset();
  }
...
class Dinosaur {
  float x, y;
  float velocity;
  float gravity = 0.9; // gravity for better jump control
  ...
  void display() {
    // Use dinosaur sprite if available, otherwise draw simple shape
    if (dinoSprite != null) {
...
class Obstacle {
  ...
  void display() {
    ...
    if (debugMode) {
      ...
      textAlign(LEFT);
      textSize(10);
      text("W:" + int(width) + " H:" + int(height), x, y - 5);
    }
  }
}
